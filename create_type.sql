...
CREATE OR REPLACE TYPE TP_INSTITUICAO_FINANCEIRA AS OBJECT (
    ...
    CONSTRUCTOR FUNCTION TP_INSTITUICAO_FINANCEIRA (x1 TP_INSTITUICAO_FINANCEIRA) RETURN SELF AS RESULT
);
/

-- Implementação do construtor
CREATE OR REPLACE TYPE BODY TP_INSTITUICAO_FINANCEIRA AS
    CONSTRUCTOR FUNCTION TP_INSTITUICAO_FINANCEIRA (x1 TP_INSTITUICAO_FINANCEIRA) RETURN SELF AS RESULT IS
    BEGIN
        CNPJ_INSTITUICAO := x1.CNPJ_INSTITUICAO;
        NOME := x1.NOME;
        SETOR := x1.SETOR;
        RETURN;
    END;
